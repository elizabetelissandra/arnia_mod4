...
export class PetService {
  ...
  async allPets(breed?: string) {
    try {
      if (breed) {
        return await this.petRepository.find({ where: { breed } });
      }
  ...
  async update(idPet: number, req: Request, body: UpdatePetDto) {
    try {
      const pet = await this.petRepository.findOne({where: {id: idPet}, relations: {user: true}})
      ...
      if(pet.user.id !== id){
        throw new UnauthorizedException('User is not authorized to update this pet.')
      }
